...
function onCreate()
	--<		  Add art functions       >--
	--tutorial:
	-- addArt('name','description','color(HEX)','sizeSelected','sizeNotSelected', 'animationName')
	-- example:	addArt('pizdec','vanya','FFA500',5,3)
	-- example:	addArt('bfCar','WOW, ANIMATED IMAGE','6666FF',5,3, "BF idle dance")

	addArt('art1','URCM','FFFFFF',5,3)
	addArt('art2','Cuadry','FFFFFF',5,3)
	addArt('art3','URCM','FFFFFF',5,3)
...
function addArt(name,description,color,sizeSelected,sizeNotSelected, animationName)
	images[length] = {
		name = name,
...
if id == 0 then
	if(images[length].animationName == noone) then
		makeLuaSprite('left2', 'gallery/'..images[length].name, 60, 100)
	else
		makeAnimatedLuaSprite('left2', 'gallery/'..images[length].name, 60, 100)
		addAnimationByPrefix('left2', 'idle', images[length].animationName, 24, true)
	end

	if(images[id].animationName == noone) then
		makeLuaSprite('Middle2', 'gallery/'..images[id].name, 380, 100)
	else
		makeAnimatedLuaSprite('Middle2', 'gallery/'..images[id].name, 380, 100)
		addAnimationByPrefix('Middle2', 'idle', images[id].animationName, 24, true)
	end

	if(images[id + 1].animationName == noone) then
		makeLuaSprite('Right2', 'gallery/'..images[id + 1].name, 280*3, 100)
	else
		makeAnimatedLuaSprite('Right2', 'gallery/'..images[id + 1].name, 280*3, 100)
		addAnimationByPrefix('Right2', 'idle', images[id + 1].animationName, 24, true)
	end
...
elseif id == length  then
	...
	if(images[id-1].animationName == noone) then
		...
		makeAnimatedLuaSprite('left2', 'gallery/'..images[id-1].name, 60, 100)
		addAnimationByPrefix('left2', 'idle', images[id-1].animationName, 24, true)
	end
	...
	if(images[0].animationName == noone) then
		makeLuaSprite('Right2', 'gallery/'..images[0].name, 280*3, 100)
	else
		makeAnimatedLuaSprite('Right2', 'gallery/'..images[0].name, 280*3, 100)
		addAnimationByPrefix('Right2', 'idle', images[0].animationName, 24, true)
	end
